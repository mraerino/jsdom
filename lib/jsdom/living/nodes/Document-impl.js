...
class DocumentImpl extends NodeImpl {
  ...
  _createElementWithCorrectElementInterface(name, namespace) {
    ...
    // Check for custom element in registry
    const CustomElementConstructor = this._defaultView.customElements.get(name);
    if (CustomElementConstructor) {
      if (!(CustomElementConstructor.prototype instanceof HTMLElement.interface)) {
        throw new TypeError();
      }
      return new CustomElementConstructor();
    }
    ...
    if (this._elementBuilders[namespace] && this._elementBuilders[namespace][name.toLowerCase()]) {
      ...
      elem = HTMLUnknownElement.create([], privateData);
    } else {
      elem = Element.create([], privateData);
    }
